...
const Game = {
      ...
      players.forEach((player, index) => {
        ...
        playerElement.innerHTML = `
          <div class="player-avatar">
            <img src="https://api.dicebear.com/6.x/avataaars/svg?seed=${player.avatar || player.id}" alt="${player.name}">
          </div>
      ...
      opponents.forEach((opponent, index) => {
        ...
        opponentElement.innerHTML = `
          <div class="opponent-avatar">
            <img src="https://api.dicebear.com/6.x/avataaars/svg?seed=${opponent.avatar || opponent.id}" alt="${opponent.name}">
          </div>
    ...
    canPlayCard(card) {
      ...
      // Cartas especiais de empilhamento (+2, +4)
    ...
    async drawCard(playerId) {
      ...
      // Passar a vez se comprou apenas uma carta
      if (cardsToDraw === 1) {
    ...
    async showUnoButton(playerId) {
      ...
      // Iniciar timer para gritar UNO automaticamente após 3 segundos
      setTimeout(async () => {
        const players = await this.getPlayers();
        const player = players.find(p => p.id === playerId);
    ...
    handleGameOver(winnerId, players) {
      ...
      winnerAvatarImg.src = `https://api.dicebear.com/6.x/avataaars/svg?seed=${winner.avatar || winner.id}`;
